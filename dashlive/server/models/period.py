...
class Period(db.Model, ModelMixin):
    ...
    start: Mapped[datetime.timedelta] = mapped_column(sa.Interval(), nullable=True)
    duration: Mapped[datetime.timedelta] = mapped_column(
        sa.Interval(), nullable=True)
    adaptation_sets: Mapped[list["AdaptationSet"]] = relationship(back_populates="period")
    ...
    def get_fields(self) -> list[str, FormInputContext]:
        ...
        start = self.get_time_input('start', self.start)
        if start['value'] == '':
            start['value'] = '00:00:00'
        fields: list[str, FormInputContext] = [
            ordering,
            pid,
            start,
