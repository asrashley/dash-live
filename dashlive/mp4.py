...
class NamedObject(with_metaclass(ABCMeta, object)):
    debug=False
...
class FieldReader(object):
    ...
    def get(self, size, field, mask=None):
        ...
        if size=='B':
            ...
            shift = 1<<asz
            value = old_div(decimal.Decimal(self.get(format_bit_sizes[bsz+asz], field)), shift)
        else:
...
class Mp4Atom(with_metaclass(ABCMeta, NamedObject)):
    ...
    def _to_json(self, exclude):
        ...
        for k,v in self._fields.items():
            if k[0] != '_' and not k in exclude:
                rv[k] = v
        if self.children:
            ch = [c.toJSON() for c in self.children]
            rv['children'] = ch
        return rv
    ...
    def create(cls, src, parent=None, options=None):
        ...
        if end is None:
            ...
            options.log.debug('Create start=%d end=%d (%d)', cursor, end, end-cursor)
        while end is None or cursor<end:
            ...
            if (src.tell() - atom.position) != atom.size:
                options.log.warning('expected %s to contain %d bytes but parsed %d bytes',
    ...
    def parse(cls, src, parent, options=None, **kwargs):
        ...
        if not size or len(size)!=4:
            if options:
                if len(size)==0:
                    ...
                    options.log.debug('Failed to read box length. pos=%d', position)
            return None
        ...
        if size == 0:
            ...
            if not size:
                if options:
                    options.log.debug('Failed to read atom size. pos=%d', position)
                return None
        if atom_type=='uuid':
    ...
    def encode(self, dest=None):
        ...
        if self._encoded is not None:
            ...
            if self.size != (4 + len(t) + len(self._encoded)):
                self.options. log.warning("Expected size %d, actual size %d", self.size,
...
class Descriptor(with_metaclass(ABCMeta, NamedObject)):
    ...
    def create(clz, src, parent, options=None, **kwargs):
        ...
        if len(encoded) < total_size:
            ...
            src.seek(p)
        kw = Desc.parse(src, parent, options=kwargs.get("options"), initial_data=d)
        tag = kw["tag"]
        del kw["tag"]
    ...
    def _to_json(self, exclude):
        ...
        if self.children:
            rv['children'] = [c.toJSON(pure=False) for c in self.children]
...
class DecoderSpecificInfo(Descriptor):
    ...
    def parse(clz, src, parent, **kwargs):
        ...
        rv["data"] = None
        if r.pos() != 8*rv["size"]:
            rv["data"] = r.data[old_div(r.pos(),8):]
        return rv
...
class AVCConfigurationBox(Mp4Atom):
    ...
    def _to_json(self, exclude):
        ...
        for param in param_sets:
            try:
                fields[param] = [Binary(a) for a in self._fields[param]]
            except KeyError:
                pass
        return fields
...
class EC3SpecificBox(Mp4Atom):
    ...
    def _to_json(self, exclude):
        ...
        rv['substreams'] = [s.toJSON() for s in self.substreams]
        return rv
...
class MediaHeaderBox(FullBox):
    ...
    def parse(clz, src, parent, **kwargs):
        ...
        rv["language"] = chr(0x60+((tmp>>10)&0x1F)) + chr(0x60+((tmp>>5)&0x1F)) + chr(0x60+(tmp&0x1F))
...
class SampleAuxiliaryInformationSizesBox(FullBox):
    ...
    def parse(clz, src, parent, **kwargs):
        rv = FullBox.parse(src, parent, **kwargs)
        if rv["flags"] & 1:
            rv["aux_info_type"] = clz.check_info_type(struct.unpack('>I',
                                                                        src.read(4))[0])
...
class CencSampleEncryptionBox(FullBox):
    def __init__(self, **kwargs):
        ...
        if "samples" in self._fields:
            self._fields["samples"] = [CencSampleAuxiliaryData(**s) for s in self._fields["samples"]]
    ...
    def parse(clz, src, parent, **kwargs):
        ...
        for i in range(num_entries):
            ...
            if size:
                s = CencSampleAuxiliaryData.parse(src, size, rv["iv_size"], rv["flags"], parent)
                rv["samples"].append(s)
        return rv
    ...
    def _to_json(self, exclude):
        ...
        try:
            ...
            pass
        if "samples" in self._fields:
            fields["samples"] = [s.toJSON() for s in self.samples]
        return fields
...
class ContentProtectionSpecificBox(FullBox):
    ...
    def _to_json(self, exclude):
        ...
        if self.version > 0:
            kids = [Binary(k, encoding=Binary.HEX) for k in self.key_ids]
            fields['key_ids'] = kids
        return fields
...
class IsoParser(object):
    def walk_atoms(self, filename, atom=None, options=None):
        ...
        try:
            ...
            if src and isinstance(filename,str):
                src.close()
        return atoms
