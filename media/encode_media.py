...
#
VIDDIR=os.path.join(os.environ("HOME"), "Downloads")
...
FRAME_SEGMENT_DURATION=SEGMENT_DURATION * FRAMERATE

#
# The MP4 timescale to use for video
TIMESCALE=FRAMERATE * 10

#
# The total duration of the final DASH stream (in seconds)
DURATION=int(((9*60 + 56.45) // SEGMENT_DURATION) * SEGMENT_DURATION)
...
def encode(width,height,bitrate):
    print "{}x{} {}Kbps".format(height, width, bitrate)
    destdir = os.path.join(DESTDIR, str(bitrate))
    ...
    if [ ! -f ${DESTDIR}/$3/${PREFIX}.mp4 ]; then
       ...
       (cd ${DESTDIR}/$3 && ffmpeg -ss 5 -ec deblock \
	       ...
	       -y -t $DURATION -threads 0 "${PREFIX}.mp4") \
       && ffprobe -show_frames -print_format compact "${DESTDIR}/$3/${PREFIX}.mp4" | awk -F '|' '($2 == "media_type=video") { \
    if ($4 == "key_frame=1") { \
        print i; \
...
package () {
    ...
    FILES=""
    for bitrate in $*; do
        FILES="${DESTDIR}/${bitrate}/${PREFIX}.mp4#video ${FILES}"
    done
    # Add AAC audio track
    FILES="${FILES} ${DESTDIR}/${1}/${PREFIX}.mp4#trackID=2:role=main"
    # Add E-AC3 audio track
    FILES="${FILES} ${DESTDIR}/${1}/${PREFIX}.mp4#trackID=3:role=alternate"
    echo $FILES
    ...
    rm ${DESTDIR}/${PREFIX}_[1-9].mp4
    for repr in 1 2 3 4 5 6 7 8 9; do
        if [ -f ${DESTDIR}/dash/dash_${repr}_.mp4 ]; then
            (cd ${DESTDIR}/dash && cat dash_${repr}_.mp4 dash_${repr}_*.m4s >${DESTDIR}/${PREFIX}_${repr}.mp4)
            ...
            (cd ${DESTDIR}/dash && cat manifest_set1_init.mp4 dash_${repr}_*.m4s >${DESTDIR}/${PREFIX}_${repr}.mp4)
