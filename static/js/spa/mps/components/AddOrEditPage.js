...
export function AddOrEditStreamPage({name, newStream=false }) {
  ...
  useEffect(() => {
    ...
    const fetchStreamIfRequired = async () => {
      ...
      if (loaded === name && state.model !== undefined) {
        return;
      }
      if (newStream && state.model === undefined) {
        console.log(JSON.stringify(blankModel));
        setLoaded(name);
  ...
  }, [apiRequests, name, state.allStreams.value]);
