...
import { renderHookWithProviders } from "../../test/renderHookWithProviders.js";
import { EndpointContext } from "../endpoints.js";
import { useMultiPeriodStream, blankModel } from "./useMultiPeriodStream.js";
...
describe("useMultiPeriodStream hook", () => {
  ...
  const Wrapper = ({ children }) => {
    return html`<${EndpointContext.Provider} value=${apiRequests}>${children}</${EndpointContext.Provider}>`;
  };
  let getMultiPeriodStreamPromise;

  beforeEach(() => {
    getMultiPeriodStreamPromise = new Promise((resolve) => {
      apiRequests.getMultiPeriodStream.mockImplementation(async () => {
        const data = await import("../../mocks/demo-mps.json");
        resolve();
        return data;
      });
    });
